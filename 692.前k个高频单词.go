/*
 * @lc app=leetcode.cn id=692 lang=golang
 *
 * [692] 前K个高频单词
 *
 * https://leetcode.cn/problems/top-k-frequent-words/description/
 *
 * algorithms
 * Medium (56.42%)
 * Likes:    548
 * Dislikes: 0
 * Total Accepted:    104.9K
 * Total Submissions: 186.1K
 * Testcase Example:  '["i","love","leetcode","i","love","coding"]\n2'
 *
 * 给定一个单词列表 words 和一个整数 k ，返回前 k 个出现次数最多的单词。
 *
 * 返回的答案应该按单词出现频率由高到低排序。如果不同的单词有相同出现频率， 按字典顺序 排序。
 *
 *
 *
 * 示例 1：
 *
 *
 * 输入: words = ["i", "love", "leetcode", "i", "love", "coding"], k = 2
 * 输出: ["i", "love"]
 * 解析: "i" 和 "love" 为出现次数最多的两个单词，均为2次。
 * ⁠   注意，按字母顺序 "i" 在 "love" 之前。
 *
 *
 * 示例 2：
 *
 *
 * 输入: ["the", "day", "is", "sunny", "the", "the", "the", "sunny", "is", "is"],
 * k = 4
 * 输出: ["the", "is", "sunny", "day"]
 * 解析: "the", "is", "sunny" 和 "day" 是出现次数最多的四个单词，
 * ⁠   出现次数依次为 4, 3, 2 和 1 次。
 *
 *
 *
 *
 * 注意：
 *
 *
 * 1 <= words.length <= 500
 * 1 <= words[i] <= 10
 * words[i] 由小写英文字母组成。
 * k 的取值范围是 [1, 不同 words[i] 的数量]
 *
 *
 *
 *
 * 进阶：尝试以 O(n log k) 时间复杂度和 O(n) 空间复杂度解决。
 *
 */

// @lc code=start
type pair struct {
	w string
	c int
}

func topKFrequent(words []string, k int) []string {
	mp := map[string]int{}
	for _, k := range words {
		mp[k]++
	}
	h := &MyHeap{}
	heap.Init(h)
	for key, v := range mp {
		heap.Push(h, pair{key, v})
		if h.Len() > k {
			heap.Pop(h)
		}
	}
	res := make([]string, k)
	for i := k - 1; i >= 0; i-- {
		res[i] = heap.Pop(h).(pair).w
	}
	return res
}

type MyHeap []pair

func (myheap MyHeap) Less(i, j int) bool {
	return myheap[i].c < myheap[j].c || (myheap[i].c == myheap[j].c && myheap[i].w > myheap[j].w)
}
func (myheap MyHeap) Len() int {
	return len(myheap)
}
func (myheap MyHeap) Swap(i, j int) {
	myheap[i], myheap[j] = myheap[j], myheap[i]
}
func (myheap *MyHeap) Push(x interface{}) {
	*myheap = append(*myheap, x.(pair))
}
func (myheap *MyHeap) Pop() interface{} {
	n := len(*myheap)
	res := (*myheap)[n-1]
	*myheap = (*myheap)[:n-1]
	return res
}

// @lc code=end

